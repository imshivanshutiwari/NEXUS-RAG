                ...
                [
                    ...
                    dbc.Col(
                        dbc.Card(
                            ...
                            children=[
                                html.H5(
                                    "Dense vs Sparse Scores", style={"color": ACCENT}
                                ),
                                dcc.Graph(
                                    id="dense-sparse-scatter", style={"height": "280px"}
                                ),
                            ],
                        ),
                        width=6,
                    ...
                    dbc.Col(
                        dbc.Card(
                            ...
                            children=[
                                ...
                                dcc.Graph(
                                    id="score-histogram", style={"height": "280px"}
                                ),
                            ],
                        ),
                        width=6,
                ...
                [
                    ...
                    dbc.Col(
                        dbc.Card(
                            ...
                            children=[
                                html.H5(
                                    "Top Retrieved Chunks", style={"color": ACCENT}
                                ),
                                dcc.Graph(
                                    id="retrieved-table", style={"height": "300px"}
                                ),
                            ],
                        ),
                        width=8,
...
@callback(
    Output("dense-sparse-scatter", "figure"),
    Input("ret-store", "data"),
    prevent_initial_call=True,
)
def scatter(data):
    ...
    fig.update_layout(
        ...
        paper_bgcolor=BG,
        font_color=TEXT,
        xaxis_title="Dense Score",
        yaxis_title="Sparse (approx)",
...
def ret_table(data):
    ...
    fig = go.Figure(
        go.Table(
            header=dict(
                values=["doc_id", "score", "source", "content"],
                fill_color=BG,
                font_color=ACCENT,
            ),
            cells=dict(
                values=[
                    [d["doc_id"][:12] for d in pre],
                    [f"{d['score']:.3f}" for d in pre],
                    [d["source"] for d in pre],
                    [d.get("content", "")[:80] for d in pre],
                ],
                fill_color=BG,
                font_color=TEXT,
            ),
    ...
    fig.update_layout(
        plot_bgcolor=BG, paper_bgcolor=BG, margin=dict(l=0, r=0, t=0, b=0)
    )
    return fig
