...
def layout():
    return dbc.Container(
        ...
        children=[
            ...
            dbc.Button(
                "Run Benchmark", id="run-bench-btn", color="warning", className="mb-3"
            ),
                ...
                [
                    ...
                    dbc.Col(
                        dbc.Card(
                            ...
                            children=[
                                html.H5(
                                    "Mean RAGAS Scores (Radar)", style={"color": ACCENT}
                                ),
                    ...
                    dbc.Col(
                        dbc.Card(
                            ...
                            children=[
                                ...
                                dcc.Graph(
                                    id="ragas-heatmap", style={"height": "300px"}
                                ),
                            ],
                        ),
                        width=6,
                ...
                [
                    ...
                    dbc.Col(
                        dbc.Card(
                            ...
                            children=[
                                html.H5(
                                    "NEXUS-RAG vs Naive Baseline",
                                    style={"color": ACCENT},
                                ),
                                dcc.Graph(
                                    id="baseline-compare", style={"height": "250px"}
                                ),
                            ],
                        ),
                        width=12,
...
def run_bench(n):
    try:
        ...
        return (
            df.to_dict("records"),
            f"Benchmark complete: {len(df)} questions evaluated.",
        )
...
def radar(data):
    ...
    fig.update_layout(
        polar=dict(bgcolor=BG, radialaxis=dict(range=[0, 1])),
        paper_bgcolor=BG,
        font_color=TEXT,
...
def violin(data):
    ...
    for m in metrics:
        if m in df.columns:
            fig.add_trace(
                go.Violin(y=df[m].tolist(), name=m, line_color=ACCENT, fillcolor=BG)
            )
    fig.update_layout(
        plot_bgcolor=BG,
        paper_bgcolor=BG,
...
@callback(
    Output("baseline-compare", "figure"),
    Input("bench-store", "data"),
    prevent_initial_call=True,
)
def baseline(data):
    ...
    nexus_means = [
        df[m].mean() if (not df.empty and m in df.columns) else 0 for m in metrics
    ]
