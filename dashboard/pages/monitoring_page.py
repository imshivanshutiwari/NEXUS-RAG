    ...
    return dbc.Container(fluid=True, style={"background": BG, "padding": "24px"}, children=[
        ...
        dbc.Row([
            ...
            dbc.Col(dbc.Card(style=CARD_STYLE, children=[
                ...
                dcc.Graph(id="rolling-means-chart", style={"height": "260px"}),
            ]), width=8),
        ...
        dbc.Row([
            ...
            dbc.Col(dbc.Card(style=CARD_STYLE, children=[
                ...
                dcc.Graph(id="faithfulness-trend", style={"height": "220px"}),
            ]), width=12),
        ]),
    ])
...
def _get_monitor_data():
    try:
        ...
        pass
    return {
        "drift_score": 0.0, "drift_status": "STABLE",
        "rolling_faithfulness": 0.82, "rolling_relevancy": 0.78,
        "latency_stats": {
...
def rolling_means(n):
    ...
    history = QualityMonitor().get_history() or [
        {"faithfulness": 0.8, "answer_relevancy": 0.75, "context_precision": 0.7, "context_recall": 0.65}
    ]
    ...
    fig.update_layout(plot_bgcolor=BG, paper_bgcolor=BG, font_color=TEXT,
                      yaxis_range=[0, 1], margin=dict(l=30, r=10, t=10, b=40))
    return fig
...
def latency_bar(n):
    ...
    fig.update_layout(barmode="group", plot_bgcolor=BG, paper_bgcolor=BG, font_color=TEXT,
                      yaxis_title="ms", margin=dict(l=30, r=10, t=10, b=40))
    return fig
...
def alert_list(n):
    ...
    return [html.Div(f"[{a.severity}] {a.name}: {a.message}", style={
...
def faithfulness_trend(n):
    ...
    threshold = 0.7
    fig = go.Figure([
        go.Scatter(y=vals, mode="lines+markers", line=dict(color=ACCENT), name="faithfulness"),
        go.Scatter(y=[threshold] * len(vals), mode="lines", line=dict(color="#ff4d4d", dash="dash"), name="threshold"),
    ])
    fig.update_layout(plot_bgcolor=BG, paper_bgcolor=BG, font_color=TEXT,
                      yaxis_range=[0, 1], margin=dict(l=30, r=10, t=10, b=40))
    return fig
