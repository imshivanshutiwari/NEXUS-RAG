                ...
                [
                    ...
                    dbc.Col(
                        dbc.Card(
                            ...
                            children=[
                                html.H5(
                                    "6-Node Pipeline Graph", style={"color": ACCENT}
                                ),
                                dcc.Graph(
                                    id="graph-topology",
                                    figure=_make_topology(),
                                    style={"height": "340px"},
                                ),
                            ],
                        ),
                ...
                [
                    ...
                    dbc.Col(
                        dbc.Card(
                            ...
                            children=[
                                ...
                                dcc.Graph(
                                    id="stage-progress", style={"height": "200px"}
                                ),
                            ],
                        ),
                        width=6,
                    ...
                    dbc.Col(
                        dbc.Card(
                            ...
                            children=[
                                html.H5(
                                    "Healing Iteration Details", style={"color": ACCENT}
                                ),
...
@callback(
    Output("stage-progress", "figure"),
    Output("healing-flow", "figure"),
    Output("trace-text", "children"),
    Input("trace-btn", "n_clicks"),
    State("graph-query", "value"),
...
def trace_query(n, query):
    ...
    try:
        ...
        fig1 = go.Figure(
            go.Bar(
                x=_NODES,
                y=[1 if s in stages_hit else 0.2 for s in _NODES],
                marker_color=colors,
                showlegend=False,
            )
        )
        ...
        fig2.update_layout(
            plot_bgcolor=BG, paper_bgcolor=BG, margin=dict(l=10, r=10, t=10, b=10)
        )
        trace_str = "\n".join(trace)
