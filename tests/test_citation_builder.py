...
def test_extract_citations(sample_contexts):
    ...
    docs = [{"doc_id": f"d{i}", "content": c, "source": "test", "chunk_index": i, "metadata": {}} for i, c in enumerate(sample_contexts)]
...
def test_no_citations(sample_contexts):
    builder = CitationBuilder()
    docs = [{"doc_id": "d0", "content": "text", "source": "test", "chunk_index": 0, "metadata": {}}]
...
def test_bibliography(sample_contexts):
    builder = CitationBuilder()
    citations = [{"citation_number": 1, "title": "Wikipedia", "url": "https://en.wikipedia.org", "source": "wiki"}]
